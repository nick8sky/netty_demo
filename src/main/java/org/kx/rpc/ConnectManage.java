...
public class ConnectManage {
    ...
    public void updateConnectedServer(List<NodeInfo> allServerAddress) {
        if(allServerAddress != null && !allServerAddress.isEmpty()){
            HashSet<InetSocketAddress> newAllServerNodeSet = new HashSet<InetSocketAddress>();
            for(NodeInfo nodeInfo :allServerAddress){
                final InetSocketAddress remotePeer = new InetSocketAddress(nodeInfo.getIp(), nodeInfo.getPort());
    ...
    public RpcClientHandler chooseHandler() {
        ...
        int index = (roundRobin.getAndAdd(1) + size) % size; //随机轮询
